...
const defaultConfigs: Record<string, Record<string, unknown>> = {
    ...
    FORM: { fields: [
        { id: 'f1', type: 'text', label: '申請内容', required: true },
        { id: 'f2', type: 'date', label: '発生日', required: true },
        { id: 'f3', type: 'textarea', label: '理由・備考', required: false }
    ], description: '' },
};
...
const renderFieldMock = (field: Field) => {
    switch (field.type) {
        ...
        case 'date':
            return <input type="date" className="w-full px-4 py-2 mt-2 bg-gray-50 border border-gray-200 rounded-lg text-sm" />
        case 'select':
            return (
                <select className="w-full px-4 py-2 mt-2 bg-gray-50 border border-gray-200 rounded-lg text-sm">
                    {field.options?.map((opt: string, i: number) => <option key={i}>{opt}</option>)}
                </select>
            );
        case 'file':
            return (
                <div className="w-full p-4 mt-2 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50 flex flex-col items-center justify-center text-gray-400">
                    <svg className="w-6 h-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                    <span className="text-[10px] font-bold uppercase tracking-wider">ファイル添付を選択</span>
                </div>
            )
        case 'number':
            return (
                <div className="flex items-center gap-3 mt-2">
                    <div className="w-10 h-10 border border-gray-200 rounded-xl flex items-center justify-center text-gray-400 bg-gray-50">-</div>
                    <div className="w-12 text-center font-bold text-gray-900 border-b-2 border-gray-100">1</div>
                    <div className="w-10 h-10 border border-gray-200 rounded-xl flex items-center justify-center text-gray-400 bg-gray-50">+</div>
                    <span className="text-xs text-gray-400 font-bold ml-1">枚</span>
                </div>
            )
        case 'checklist_item':
            return (
    ...
    return (
                ...
                try {
                    ...
                    if (data.action === 'reorder') {
                        ...
                        const newField = {
                            ...
                            required: false,
                            options: (data.type === 'radio' || data.type === 'checkbox' || data.type === 'select') ? ['選択肢1', '選択肢2'] : undefined,
                            min: data.type === 'number' ? 1 : undefined,
                            max: data.type === 'number' ? 10 : undefined,
                        };
                        const fields = [...((newWidget.configJson?.fields as Field[]) || [])];
        ...
        >
            ...
            <div className="pl-6">
                ...
                {field.type === 'number' && (
                    <div className="mt-3 bg-gray-50 rounded-lg p-3 space-y-3 border border-orange-100">
                        ...
                        <div className="flex gap-4">
                            <div className="flex-1">
                                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">下限値</label>
                                <input
                                    type="number"
                                    value={field.min ?? 0}
                                    onChange={(e) => updateField(field.id, { min: parseInt(e.target.value) || 0 })}
                                    className="text-xs w-full bg-white border border-gray-200 rounded px-2 py-1 focus:ring-1 focus:ring-orange-500 focus:outline-none font-bold"
                                />
                            </div>
                            <div className="flex-1">
                                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">上限値</label>
                                <input
                                    type="number"
                                    value={field.max ?? 100}
...
export default function NewMenuWidgetPage() {
    ...
    const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        else if (type === 'FORM') initialFields.push(
            { id: 'f1', type: 'text', label: 'お名前', required: true },
            { id: 'f2', type: 'date', label: '発生日', required: true },
            { id: 'f3', type: 'radio', label: '申請項目', options: ['結婚', '出産', 'そのほか'] }
        );
    ...
    const getAvailableComponents = () => {
        switch (newWidget.type) {
            ...
            case 'FORM': return [
                { type: 'text', label: '1行テキスト', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
                { type: 'textarea', label: '複数行テキスト', icon: 'M4 6h16M4 12h16M4 18h7' },
                { type: 'date', label: '日付選択', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
                { type: 'radio', label: '単一選択(ボタン型)', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
                { type: 'select', label: 'ドロップダウン(リスト型)', icon: 'M19 9l-7 7-7-7' },
                { type: 'number', label: '枚数・個数(カウンター型)', icon: 'M12 6v6m0 0v6m0-6h6m-6 0H6' },
                { type: 'file', label: '添付書類(画像など)', icon: 'M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-8l-4-4m0 0L8 8m4-4v12' },
            ];
            default: return [
                { type: 'text', label: '1行テキスト', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
            ];
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/menu/widget`, {
                method: 'POST',
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <h1 className="text-2xl font-black text-gray-900 mt-2">メニューへの追加</h1>
                    </div>
                </div>
                                        ...
                                        <div className="bg-white rounded-3xl border border-gray-200 overflow-hidden shadow-sm">
                                            ...
                                            {newWidget.image ? (
                                                <div className="w-full h-40 bg-gray-100">
                                                    ...
                                                    <img src={URL.createObjectURL(newWidget.image)} alt="Header" className="w-full h-full object-cover" />
                                                </div>
                                            ) : (
                                                        ...
                                                        {newWidget.type === 'SURVEY' ? '回答を入力する' :
                                                            newWidget.type === 'ATTENDANCE' ? '出欠を登録する' :
                                                                newWidget.type === 'CHECKLIST' ? 'チェックリストを保存' :
                                                                    newWidget.type === 'BOARD' ? '掲示板に投稿する' :
                                                                        newWidget.type === 'FORM' ? '申請内容を送信する' : 'アクションボタン'}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
